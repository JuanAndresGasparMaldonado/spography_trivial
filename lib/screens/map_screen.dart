...
class _MapScreenState extends State<MapScreen> {
        ...
        child: ListView(
          ...
          children: [
            UserAccountsDrawerHeader(
              ...
              accountName: Text("Usuario", style: GoogleFonts.montserrat(fontWeight: FontWeight.bold)),
        ...
        decoration: BoxDecoration(
          ...
          boxShadow: [
            ...
            BoxShadow(color: Colors.black.withValues(alpha: 0.5), blurRadius: 10, offset: const Offset(0, 5)),
             // Brillo azul
            BoxShadow(color: const Color(0xFF2196F3).withValues(alpha: 0.2), blurRadius: 20), 
          ],
        ),
        child: Row(
  ...
  Widget _barraNavIconos(IconData icon, int index) {
    ...
    if (isSelected && index != 3) {
      iconColor = const Color(0xFF2196F3); // AZUL si está seleccionado (menos el menú)
    } else if (isProminent) {
      iconColor = Colors.white; // BLANCO si es Copa/Mapa pero no está seleccionado
    } else {
      ...
      child: Center(
        child: Column(
          ...
          children: [
            ...
            Text("(Próximamente)", style: GoogleFonts.montserrat(color: Colors.white54, fontSize: 16)),
          ],
        ),
      ),
    );
